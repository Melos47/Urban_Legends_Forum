...
function renderNotificationPagination() {
    ...
    prevBtn.className = 'brutalist-btn';
    prevBtn.textContent = '◀';
    ...
    nextBtn.className = 'brutalist-btn';
    nextBtn.textContent = '▶';
...
function renderPagination() {
    ...
    if (pagination.has_prev) {
        html += `<button class="brutalist-btn" onclick="changePage(${pagination.prev_page})">◀ PREV</button>`;
    } else {
        html += `<button class="brutalist-btn" disabled style="opacity: 0.5;">◀ PREV</button>`;
    }
    
    // 页码信息
    html += `<span style="margin: 0 15px; color: #000; font-weight: bold; font-family: var(--font-mono);">PAGE ${pagination.page} / ${pagination.pages}</span>`;
    
    // 下一页按钮
    if (pagination.has_next) {
        html += `<button class="brutalist-btn" onclick="changePage(${pagination.next_page})">NEXT ▶</button>`;
    } else {
        html += `<button class="brutalist-btn" disabled style="opacity: 0.5;">NEXT ▶</button>`;
    }
...
async function showStoryDetail(storyId) {
    try {
        ...
        if (isLocked) {
            ...
            html += '<div style="margin-top: 12px; padding-top: 12px; border-top: 2px solid #000;">' +
                '<form onsubmit="submitComment(event, ' + storyId + ')">' +
                '<textarea id="comment-text" placeholder="ENTER YOUR THOUGHTS..." style="width:100%; height:60px; padding:8px; border:2px solid #000; font-size:12px; resize:none; font-family: var(--font-mono); background: #fff; color: #000;"></textarea>' +
                '<button type="submit" class="brutalist-btn" style="margin-top:8px; width:100%;">POST COMMENT</button>' +
                '</form></div>';
        } else {
            html += '<p style="text-align:center; color:#000; margin-top:12px;"><a href="#" onclick="showLoginForm(); return false;" style="color:#000; text-decoration: underline; font-weight: bold;">LOGIN</a> TO COMMENT</p>';
        }
...
function showReplyBox(commentId, authorName) {
    ...
    if (replyBox.style.display === 'none' || !replyBox.innerHTML) {
        replyBox.innerHTML = '<form onsubmit="submitReply(event, ' + commentId + ')" style="margin-top: 8px;">' +
            '<div style="color: #000; font-size: 10px; margin-bottom: 4px; font-weight: bold;">REPLY TO @' + escapeHtml(authorName) + ':</div>' +
            '<textarea id="reply-text-' + commentId + '" placeholder="ENTER REPLY..." style="width:100%; height:50px; padding:6px; border:2px solid #000; font-size:12px; resize:none; font-family: var(--font-mono); background: #fff; color: #000;"></textarea>' +
            '<div style="margin-top: 6px;">' +
            '<button type="submit" class="brutalist-btn" style="font-size: 10px; padding: 4px 12px;">SEND</button> ' +
            '<button type="button" onclick="hideReplyBox(' + commentId + ')" class="brutalist-btn" style="font-size: 10px; padding: 4px 12px;">CANCEL</button>' +
            '</div></form>';
